...
public class StickMan: MonoBehaviour {
    ...
    void Update ()
    {
        // Decrease death respawn timer  리스폰 타이머 
        m_respawnTimer -= Time.deltaTime;

        // Increase timer that controls attack combo 어택 콤보 타이머
        m_timeSinceAttack += Time.deltaTime;

        // Decrease timer that checks if we are in parry stance 패링 타이머 감소체크 
        m_parryTimer -= Time.deltaTime;

        // Decrease timer that disables input movement. Used when attacking 공격시 움직임 입력 비활성 타이머
        m_disableMovementTimer -= Time.deltaTime;

        // Respawn Hero if dead 부활 코드 
        ...
        //Check if character just landed on the ground 케릭터 땅에 있을경우 체크 
        ...
        //Check if character just started falling  케릭터 떨어졌는지 체크
        if (m_grounded && !m_groundSensor.State())
        {
            m_grounded = false;
            m_animator.SetBool("Grounded", m_grounded);
        ...
        // -- Handle input and movement -- 입력 및 이동 처리 
        float inputX = 0.0f;

        if (m_disableMovementTimer < 0.0f)
            inputX = Input.GetAxis("Horizontal");

        // GetAxisRaw returns either -1, 0 or 1  방향키 입력 -1.0.1 
        float inputRaw = Input.GetAxisRaw("Horizontal");

        // Check if character is currently moving 케릭터 현재 이동중인지 체크 
        if (Mathf.Abs(inputRaw) > Mathf.Epsilon && Mathf.Sign(inputRaw) == m_facingDirection)
            m_moving = true;
        else
            m_moving = false;

        // Swap direction of sprite depending on move direction  이동 방향에 따라 스프라이트 방향 바꾸기
        ...
        // SlowDownSpeed helps decelerate the characters when stopping 감속
        ...
        // Set AirSpeed in animator AirSpeed 애니메이션 설정
        m_animator.SetFloat("AirSpeedY", m_body2d.velocity.y);

        // Set Animation layer for hiding sword 착검 애니메이션 설정
        ...
        if (m_wallSensorR1 && m_wallSensorR2 && m_wallSensorL1 && m_wallSensorL2)
        {
            ...
            //Play wall slide sound 벽슬라이딩 사운드 재생
        ...
        */
        ...
        // Dodge 닷지/회피
        if (Input.GetKeyDown("x") && m_grounded && !m_dodging && !m_ledgeGrab && !m_ledgeClimb)
        {
            m_dodging = true;
            m_crouching = false;
        ...
        //Walk 걷기
        else if (m_moving && Input.GetKey(KeyCode.LeftShift))
        {
    ...
    // Function used to spawn a dust effect 모래 이팩트 사용함수
    // All dust effects spawns on the floor 플로어 위에 모든모래이팩트 생서됨
